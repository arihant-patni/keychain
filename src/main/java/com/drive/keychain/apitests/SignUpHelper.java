    ...
    public SignUp signUp(SignUp signUp) {
        ...
        try {
            JsonObject response = httpClient.send(
                    ...
                    false   // authRequired (assuming sign up does not require auth, but set to true to trigger AuthService if needed)
            );

            LOGGER.info("Request body for login: {}", objectToJsonString(signUp));
            LOGGER.info("Received response for login: {}", response);
    ...
    public SignUp login(SignUp signUp) {
        ...
        try {
            JsonObject response = httpClient.send(
                    ...
                    null,  // authorizationKey
                    true,   // isHttps (JSONPlaceholder uses HTTPS)
                    true   // authRequired (assuming sign up does not require auth, but set to true to trigger AuthService if needed)
            );
            LOGGER.info("Request body for login: {}", objectToJsonString(signUp));
            LOGGER.info("Received response for login: {}", response);
